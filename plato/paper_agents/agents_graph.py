...
from .scientific_verifier import scientific_verifier_node
...
def _claim_evidence_fanout(state: GraphState):
    ...
    if not files.get("f_stream"):
        folder = cast(Any, files.get("Folder") or files.get("Paper_folder"))
        if folder:
...
def build_graph(mermaid_diagram=False, checkpointer=None):
    ...
    builder.add_node("scientific_verifier_node", scientific_verifier_node)
