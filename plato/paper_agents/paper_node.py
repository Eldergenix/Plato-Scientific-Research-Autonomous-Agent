...
def keywords_node(state: GraphState, config: RunnableConfig):
    ...
    params = cast(dict[str, Any], dict(state["params"]))
    num_keywords = int(params.get("num_keywords", 0))
    ...
    if f_temp.exists():
        ...
        if state["paper"]["cmbagent_keywords"]:
            ...
            paper_config = cast(dict[str, Any], state["paper"])
            extractor_name = cast(
                str,
                paper_config.get("keyword_extractor")
            ...
            for attempt in range(3):  # try 3 times in case it fails
                ...
                keywords = matched_keywords_text
                ...
                if len(keyword_items) >= num_keywords:
                    break
            else:
            ...
            keyword_items = random.sample(keyword_items, num_keywords)
    ...
    print(" |  done ", end="")
    print(
        f"{state['tokens']['ti']} {state['tokens']['to']} [{int(minutes)}m {int(seconds)}s]"
    )
    # print(f"  Selected keywords: {keywords} {state['tokens']['ti']} {state['tokens']['to']} [{int(minutes)}m {int(seconds)}s]")

    return {
        "paper": {**state["paper"], "Keywords": keywords},
        "params": params,
        "tokens": state["tokens"],
    }
...
def abstract_node(state: GraphState, config: RunnableConfig):
    ...
    if f_temp1.exists():
        ...
        writer.write(
            "temp/Title.tex", _wrap_latex_section(state, state["paper"]["Title"])
        )
        writer.write(
            "temp/Abstract.tex", _wrap_latex_section(state, state["paper"]["Abstract"])
        )
        ...
        success = compile_tex_document(
            state, f_temp1, state["files"]["Temp"]
        )  # abstract
        if not (success):
            state["latex"]["section_to_fix"] = "Abstract"
            state, fixed = fix_latex(state, f_temp1)
            if not fixed:
                raise RuntimeError("Unable to fix LaTeX errors in the abstract.")
...
def section_node(
    ...
    paper = cast(dict[str, Any], state["paper"])

    # check if abstract already exists
    if f_temp.exists():
        paper[section_name] = temp_file(state, f_temp, "read")
        ...
        for attempt in range(3):
            ...
            paper[section_name] = section_text

            # --- Step 2: Optional self-reflection ---
            if reflection_fn:
                for _ in range(2):
                    PROMPT = reflection_fn(state)
                    state, result = LLM_call(
                        PROMPT, state, node_name=f"{_node_name}_reflection"
                    )
                    section_text = extract_latex_block(state, result, section_name)
            ...
            paper[section_name] = clean_section(section_text, section_name)
            ...
            if scope is not None:
                ...
                writer.write(
                    ...
                    _wrap_latex_section(state, paper[section_name]),
                )
            else:
                temp_file(state, f_temp, "write", paper[section_name])

            # --- Step 6: Compile and try to fix LaTeX errors ---
            if compile_tex_document(
                state, f_temp, state["files"]["Temp"]
            ):  # returns True if compiled properly
                break
            else:
                ...
                if (
                    fixed
                ):  # if fixed=True it means it fixed the error and compiled properly
                    break
    ...
    return {
        "paper": {**state["paper"], section_name: paper[section_name]},
        "tokens": state["tokens"],
    }
...
def plots_node(state: GraphState, config: RunnableConfig):
    ...
    for start in range(0, num_images, batch_size):
        ...
        # get the caption for each image in the batch
            ...
            for i, file in enumerate(
                tqdm(
                    batch_files,
                    desc=f"Processing figures {start + 1}-{min(start + batch_size, num_images)}",
                )
            ):
        ...
        # temporary file with the images
        print(
            f"   Inserting figures {start + 1}-{min(start + batch_size, num_images)}".ljust(
                33, "."
            ),
            end="",
            flush=True,
        ...
        minutes, seconds = divmod(time.time() - state["time"]["start"], 60)
        print(
            f" |  done {state['tokens']['ti']} {state['tokens']['to']} [{int(minutes)}m {int(seconds)}s]"
        )
    ...
    if num_images > 0:
        ...
        if not (success):
            ...
            state, fixed = fix_latex(state, f_temp)
            if not fixed:
                raise RuntimeError("Unable to fix LaTeX errors in the results.")
        minutes, seconds = divmod(time.time() - state["time"]["start"], 60)
        print(
            f" |  done {state['tokens']['ti']} {state['tokens']['to']} [{int(minutes)}m {int(seconds)}s]"
        )
    ...
    return {
        "paper": {**state["paper"], "Results": state["paper"]["Results"]},
        "tokens": state["tokens"],
    }
...
async def citations_node(state: GraphState, config: RunnableConfig):
    ...
    for section_name, updated_text, references in results:
        paper[section_name] = updated_text
    ...
    process_bib_file(
        f"{state['files']['Paper_folder']}/bibliography_temp.bib",
    ...
    for section_name in sections:
        ...
        if f_temp.exists():
            ...
            PROMPT = clean_section_prompt(state, paper[section_name])
