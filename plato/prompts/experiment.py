...
When the project requires reference DNA, annotation overlap, VCF, variant-aware sequence, motif, or track queries, instruct the engineer to use the GenomeKit-backed genomics tool registry rather than handwritten coordinate parsing:

```python
from plato.tools import call
from plato.tools.builtin import GenomeKitQueryInput
...
print(result.execution.stdout if result.execution else result.model_dump_json())
```
...
- Prefer Plato's genomics tool registry for GenomeKit-backed resource access when applicable. Import `GenomeKitQueryInput` from `plato.tools.builtin` and call `prepare_genomekit_query` for reference sequence, annotation overlap, VCF, variant-sequence, motif, or track queries. Print the operation, genome build, DNA0 interval, resource path, JSON stdout, missing requirements, and cache/download warnings.
