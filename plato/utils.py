...
def llm_parser(llm: LLM | str) -> LLM:
    ...
    if isinstance(llm, str):
        try:
            ...
            raise KeyError(
                f"LLM '{llm}' not available. Please select from: {list(models.keys())}"
            )
    return llm
...
def get_task_result(chat_history: list[dict[str, Any]], name: str) -> str | None:
...
def in_notebook() -> bool:
    ...
    try:
        ...
        shell = get_ipython()
        if shell is None or "IPKernelApp" not in shell.config:  # pragma: no cover
            return False
    except ImportError:
        return False
