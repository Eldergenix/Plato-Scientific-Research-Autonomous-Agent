...
class Method:
    ...
    def __init__(
        ...
        # Set prompts
        self.planner_append_instructions = method_planner_prompt.format(
            research_idea=research_idea
        )
        self.researcher_append_instructions = method_researcher_prompt.format(
            research_idea=research_idea
        )
    ...
    def develop_method(self, data_description: str) -> str:
        ...
        if task_result is None:
            raise ValueError("researcher_response_formatter returned no task result")

        MD_CODE_BLOCK_PATTERN = r"```[ \t]*(?:markdown)[ \t]*\r?\n(.*)\r?\n[ \t]*```"
        matches = re.findall(MD_CODE_BLOCK_PATTERN, task_result, flags=re.DOTALL)
        if not matches:
            raise ValueError(
                "researcher_response_formatter output did not contain a markdown code block"
            )
